...
--
-- These functions do the filling in of oxygen (amongst other related actions)
--

-- it's actually c-1 steps as the final step had nothing to fill
doOxygen :: [Int] -> Int
doOxygen opcodes =
    let m = exploreMap opcodes
        (c, _) = flowOxygen m
     in c-1

exploreMap :: [Int] -> H.HashMap Coord Int
exploreMap opcodes =
    let r = runEngineUpdateRobot North (newRobot opcodes)
     in H.map clearRobot (r ^. robotCoords)
  where
      clearRobot v = if v == 2 then 0 else v


-- iterate the map until all the spaces are full of oxygen
flowOxygen :: H.HashMap Coord Int -> (Int, H.HashMap Coord Int)
flowOxygen m = let (as, (_,m'):_) = span fst $ iterate iterator (True, m)
                in (length as, m')
  where
    iterator :: (Bool, H.HashMap Coord Int) -> (Bool, H.HashMap Coord Int)
    iterator (_, m) = flowOxygenStep m


-- do an iteration of the map trying to fill in more oxygen.  Returns the new
-- map and a Boolean as to whether any new oxygens were done.
flowOxygenStep :: H.HashMap Coord Int -> (Bool, H.HashMap Coord Int)
flowOxygenStep m = H.foldrWithKey (checkAdjacent m) (False, H.empty) m


checkAdjacent :: H.HashMap Coord Int -> Coord -> Int -> (Bool, H.HashMap Coord Int) -> (Bool, H.HashMap Coord Int)
checkAdjacent m xy current (flowed, out) =
    let around = length $ filter (==3) $ map (lookAround m xy) [North,East,West,South]
     in if current == 0 && around > 0
          then (True, H.insert xy 3 out)
          else (flowed, H.insert xy current out)


lookAround :: H.HashMap Coord Int -> Coord -> Direction -> Int
lookAround m xy d = H.lookupDefault 0 (xy + deltaForDirection d) m


drawMap :: H.HashMap Coord Int -> String
drawMap = drawWith day15toChar (-1)


-- test functions
testIt :: Int -> H.HashMap Coord Int -> IO (H.HashMap Coord Int)
testIt c m = do
    let (f, m') = flowOxygenStep m
    putStrLn $ drawMap m
    putStrLn $ "The output was: " ++ show f ++ ", Step: " ++ show c
    getLine
    testIt (c+1) m'
...
main30 :: IO ()
main30 = do
    putStrLn "AOC2019 Day 15 Part 2 - Filling the map with oxygen"
    opcodes <- loadOpcodes opcodesFile
    {-let r = newRobot opcodes-}
        {-m = exploreMap opcodes-}
    {-final <- testIt 0 m-}
    let c = doOxygen opcodes
    putStrLn $ "Time to fill the map with Oxygen=" ++ show c
