...
runPainter :: Machine -> PaintState
runPainter m =
    let sqs = H.empty :: H.HashMap (Int,Int) Int
        pos = (0,0)
        dir = North
...
main21 :: IO ()
main21 = do
    putStrLn "AOC2019 Day 11 Part 1 - Space Police"
    opcodes <- loadOpcodes
    let m = loadMachine opcodes
        ps = runPainter m
    putStrLn $ "The length of the coords is " ++ show (length (H.toList (coords ps)))
